...
data_df <- group_nn_metrics(data_list = data_dfs,
                            ...
                            verbose = TRUE,
                            parallelize = FALSE,
                            add_coords = TRUE # could be set to FAL
                            )

tail(data_df)
```

Based on these pairwise measurements, the relative position of neighbours can be plotted:

```{r message=FALSE, warning=FALSE}
# 
# pal <- wesanderson::wes_palette('Zissou1', 10, 'continuous')
# 
# ggplot2::ggplot(data_df[data_df$nnd < 200,], ggplot2::aes(x = nnx, y = nny), color = 'black') +
...
#     contour = F
#   ) +
#   ggplot2::labs(x = 'X', y = 'Y', title = 'Sticklebacks',  fill = 'Density') +
#   ggplot2::scale_fill_gradientn(colours=pal) +
#  # ggplot2::coord_polar(start = pi, theta = 'x')+
#   ggplot2::geom_point(x = 0, y = 0, size = 3, shape = 17 ) +
#   ggplot2::theme_bw() +
...
The duration of each event is also added to the result dataframe. We suggest filtering out events of very small duration. The calculated metrics are: 
1. _mean_mean_nnd_: the temporal average of the group's average nearest neighbour distance
2. _mean_sd_nnd_: the temporal average of the group's standard deviation in nearest neighbour distance
3. _sd_mean_nnd_: the temporal standard deviation of the group's average nearest neighbor distance     
4. _mean_pol_: the average of the group's polarization during the event
5. _sd_pol_: the standard deviation of the group's polarization during the event
6. _stdv_speed_: the CV (coefficient of variation) of the group's average speed during the event
7. _mean_sd_front_: the average standard deviation of the individuals' frontness to their nearest neighbor during an event
8. _mean_mean_bangl_: the temporal average of the group's average angle
9. _mean_shape_: the average shape index of the group during an event (0= perfectly wide and 1= perfectly long relative to the heading direction)
10. _sd_mean_shape_: the standard deviation of the group's average shape index during an event.
