...
data_dfs <- group_motion_timeseries(data_df, 
                                  lonlat = FALSE, 
                                  verbose = TRUE
                                  )
...
g_metr <- calc_global_group_metrics(data_list = data_dfs,
                                   mov_av_time_window = smoothing_time_window,
                                   lonlat = FALSE,
                                   return_df = TRUE,
                                   out_csv_dir = NA)

data_df <- nn_analysis(data_list = data_dfs, 
                       lonlat = FALSE,
                       verbose = TRUE)
...
ev_summary <- events_dur_summary(data_df, step2time = 1)
