...
pick_events_threshold <- function(data_distr, var, interactive = TRUE, input_threshold = NA)
{
  if (interactive)
  {
    print(round(stats::quantile(data_distr, seq(0,1,0.05),na.rm = T), 2))
...
#' @description Adds a keep TRUE/FALSE column in the input dataframe based on the input thresholds
#' @param df A dataframe with a pol_av and speed_av columns for polarization and speed
...
#' @param step2time Sampling frequency, relation between time steps and real time
#' @return the dataframe with a keep column added. Also prints the number and duration of events defined.
#' @author Marina Papadopoulou \email{m.papadopoulou.rug@@gmail.com}
#' @export
define_events <- function(df, sp_lim, pol_lim, step2time)
{
  ...
  print(paste0(calc_events_N(df), ' events, over ', calc_events_duration(df, step2time = step2time)/60, ' min.'))
  return(df)
}
