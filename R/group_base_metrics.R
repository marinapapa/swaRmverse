...
#'  set, time, pol, speed, shape, N (number of individuals), missing_ind (whether some individuals are missing).
...
group_props_calc <- function(x, lonlat) {
  ...
  if (nrow(x) < 1 ){
    return(data.frame(set = day, t = t,pol = NA, speed = NA, shape = NA, N = N, missing_ind = NA))}

  Nnew <- length(unique(x$id))
  missing_ind <- N-Nnew
  N <- Nnew

  if (N < 2) {return(data.frame(set = day, t = t, pol = NA, speed = NA, shape = NA, N = 1, missing_ind = NA))}
  ...
  df <- data.frame(set = day, t = t, pol = D, speed = av_speed, shape = as.numeric(obb$shape), N = N, missing_ind = missing_ind)
  return(df)
}
