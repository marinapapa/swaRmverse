...

{
  ...
  if (space_type == 'pca')
  {
    my.pca <- stats::prcomp(topca[,names(topca) != 'species'], center = TRUE, scale. = TRUE)
    pca_data <- data.frame(my.pca$species, my.pca$x[,1:3])
    #save(my.pca, file = '../data/pca_res.Rdata')
    return(pca_data)
  }
  ...
  if (space_type == 'tsne')
  {
    ...
    tsne <- Rtsne::Rtsne(topca, dims = 2, perplexity = tsne_perplexity, verbose = TRUE, max_iter = 10000)
...
)
{
  ...
  pcadata <- pcadata[, c(1,3,4)]
  newpca <- newpca[, c(10, 1,2)]
  alldata <- rbind(pcadata, newpca)
  return(alldata)
}
